...
namespace WikiConnect\MediawikiApi\Client\Action;
...
class ActionApi implements Requester, LoggerAwareInterface {
	...
	public function verifyLogin(): bool {
	    $request = ActionRequest::simpleGet( 'query', ['meta' => 'userinfo'] );
	    $request->setParam( 'format', 'json' );
	    $response = $this->decodeResponse( $this->getClient()->request(
			$request->getMethod(),
			$this->apiUrl,
	...
	private function getUserAgent(): string {
		if ( !$this->auth instanceof NoAuth ) {
			if ( $this->auth instanceof UserAndPassword || $this->auth instanceof UserAndPasswordWithDomain ) {
				return 'wikiconnect-mediawiki-client/' . $this->auth->getUsername();
			}
			return 'wikiconnect-mediawiki-client/SomeUnknownUser?';
		}
		return 'wikiconnect-mediawiki-client';
	}
